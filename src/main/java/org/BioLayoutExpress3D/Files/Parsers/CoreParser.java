...
public class CoreParser
{
    ...
    */
    ...
    private void updateVertexProperties()
    {
	    ...
	    // If there's nothing there, z = 0
            if ( field4.isEmpty() )
                field4 = "0.0";

            if (DEBUG_BUILD) println("name: " + field1 + " coord1: " + field2 + " coord2: " + field2);

	    // If there's something in field5, we have four coordinates, so call updateVertexLocation for 4D
	    if ( !field5.isEmpty() )
		    nc.updateVertexLocation( field1, Float.parseFloat(field2), Float.parseFloat(field3), Float.parseFloat(field4), Float.parseFloat(field5) );
	    else
