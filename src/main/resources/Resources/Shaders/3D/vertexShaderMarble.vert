...
uniform bool marbleSphericalMapping;
uniform float marbleMorphing;
uniform bool marbleUserClipping;
...
void main()
{
    ...
    else if (marbleMorphing != 0.0)
    {
        vertex = applyMorphing(marbleMorphing, marbleTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (marbleTexturing)
        gl_TexCoord[0] = (marbleSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}