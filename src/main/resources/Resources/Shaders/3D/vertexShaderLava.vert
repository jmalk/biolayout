...
uniform bool lavaSphericalMapping;
...
void main()
{
    ...
    {
        vertex = applyMorphing(lavaMorphing, lavaTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (lavaTexturing)
        gl_TexCoord[0] = (lavaSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}