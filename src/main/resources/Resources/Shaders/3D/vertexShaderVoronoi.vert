...
uniform bool voronoiSphericalMapping;
uniform float voronoiMorphing;
uniform bool voronoiUserClipping;
...
void main()
{
    ...
    else if (voronoiMorphing != 0.0)
    {
        vertex = applyMorphing(voronoiMorphing, voronoiTimer);
    }
    else
    {
        vertex = gl_Vertex;
   ...
   gl_Position = gl_ModelViewProjectionMatrix * vertex;
    ...
    // note: texture coords always needed for this effect for the voronoi 2d texture in the fragment shader
    // if (voronoiTexturing)
    gl_TexCoord[0] = (voronoiSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}