...
uniform bool toonSphericalMapping;
uniform float toonMorphing;
uniform bool toonUserClipping;
uniform float toonTimer;
...
void main()
{
    ...
    else if (toonMorphing != 0.0)
    {
        vertex = applyMorphing(toonMorphing, toonTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (toonTexturing)
        gl_TexCoord[0] = (toonSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}