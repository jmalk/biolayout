...
uniform bool glyphBombingSphericalMapping;
uniform float glyphBombingMorphing;
uniform bool glyphBombingUserClipping;
...
void main()
{
    ...
    else if (glyphBombingMorphing != 0.0)
    {
        vertex = applyMorphing(glyphBombingMorphing, glyphBombingTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    // note: texture coords always needed for this effect for the glyphbombing texture in the fragment shader
    if (glyphBombingTexturing)
    {
        gl_TexCoord[0] = (glyphBombingSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
        VS_TEX_COORD = gl_TexCoord[0].st * ScaleFactor;
    }
    else
    {
        VS_TEX_COORD = ( (glyphBombingSphericalMapping) ? applySphericalCoordinates(VS_POSITION, VS_NORMAL) : (gl_TextureMatrix[0] * gl_MultiTexCoord0).st ) * ScaleFactor;
    }
}