...
uniform bool woodSphericalMapping;
uniform float woodMorphing;
uniform bool woodUserClipping;
...
void main()
{
    ...
    else if (woodMorphing != 0.0)
    {
        vertex = applyMorphing(woodMorphing, woodTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (woodTexturing)
        gl_TexCoord[0] = (woodSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}