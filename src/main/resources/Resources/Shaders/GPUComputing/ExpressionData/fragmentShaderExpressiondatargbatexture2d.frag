...
vec2 getNormalizedTexCoordFrom1DIndex(in int);
float calculateCorrelation(in float, in int, in int);

void main()
{
    ivec4 indicesX = ivec4( texture2D(textureX, gl_TexCoord[0].st) );
    ivec4 indicesY = ivec4( texture2D(textureY, gl_TexCoord[0].st) );
...
{
    return texture2D( textureSumX_cache, getNormalizedTexCoordFrom1DIndex(indexX) )[indexX & RGBA_VALUES_PER_TEXEL_FOR_BITSHIFT_MODULO];
}

float getOneDimensionalMatrix2FloatValueFromIndex(in int indexX)
{
    return texture2D( textureSumX_sumX2_cache, getNormalizedTexCoordFrom1DIndex(indexX) )[indexX & RGBA_VALUES_PER_TEXEL_FOR_BITSHIFT_MODULO];
}

float getOneDimensionalMatrix3FloatValueFromIndex(in int indexX)
{
    return texture2D( textureSumColumns_X2_cache, getNormalizedTexCoordFrom1DIndex(indexX) )[indexX & RGBA_VALUES_PER_TEXEL_FOR_BITSHIFT_MODULO];
}

vec2 getNormalizedTexCoordFrom1DIndex(in int indexX)
{
    ...
    return vec2( float(rowIndex), float(columnIndex) ) / float(oneDimensionalExpressionDataConvertedTo2DSquareTextureSize) + texelCenters[0];
}
...
float calculateCorrelation(in float denominator, in int indexX, in int indexY)
{
    ...
    for (int i = 0; i < totalColumns; i++)
    {
        ...
        #if TWO_DIMENSIONAL_EXPRESSION_DATA_POWER_OF_TWO
            normalizedTexCoordFrom2DIndices = vec4(texelRowColumnIndices & twoDimensionalExpressionDataConvertedTo2DSquareTextureSizeModuloDivision, texelRowColumnIndices >> twoDimensionalExpressionDataConvertedTo2DSquareTextureSizePowerOfTwo) / float(twoDimensionalExpressionDataConvertedTo2DSquareTextureSize) + texelCenters[1];
        #else
            normalizedTexCoordFrom2DIndices = vec4(texelRowColumnIndices % twoDimensionalExpressionDataConvertedTo2DSquareTextureSize, texelRowColumnIndices / twoDimensionalExpressionDataConvertedTo2DSquareTextureSize) / float(twoDimensionalExpressionDataConvertedTo2DSquareTextureSize) + texelCenters[1];
        #endif
        sumXY += ( texture2D(textureExpressionMatrix, normalizedTexCoordFrom2DIndices.xz)[texelIndices.x & RGBA_VALUES_PER_TEXEL_FOR_BITSHIFT_MODULO] * texture2D(textureExpressionMatrix, normalizedTexCoordFrom2DIndices.yw)[texelIndices.y & RGBA_VALUES_PER_TEXEL_FOR_BITSHIFT_MODULO] );
    }
