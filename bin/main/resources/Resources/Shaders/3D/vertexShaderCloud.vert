...
uniform bool cloudSphericalMapping;
uniform float cloudMorphing;
uniform bool cloudUserClipping;
...
void main()
{
    ...
    else if (cloudMorphing != 0.0)
    {
        vertex = applyMorphing(cloudMorphing, cloudTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (cloudTexturing)
        gl_TexCoord[0] = (cloudSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}