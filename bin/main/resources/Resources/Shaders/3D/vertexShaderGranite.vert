...
uniform bool graniteSphericalMapping;
uniform float graniteMorphing;
uniform bool graniteUserClipping;
...
void main()
{
    ...
    else if (graniteMorphing != 0.0)
    {
        vertex = applyMorphing(graniteMorphing, graniteTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (graniteTexturing)
        gl_TexCoord[0] = (graniteSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}