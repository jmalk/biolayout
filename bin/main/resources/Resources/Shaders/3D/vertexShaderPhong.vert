...
uniform bool phongSphericalMapping;
uniform float phongMorphing;
uniform bool phongUserClipping;
uniform float phongTimer;
...
void main()
{
    ...
    else if (phongMorphing != 0.0)
    {
        vertex = applyMorphing(phongMorphing, phongTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    if (phongTexturing)
        gl_TexCoord[0] = (phongSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}