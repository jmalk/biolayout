...
uniform bool bumpSphericalMapping;
uniform float bumpMorphing;
uniform bool bumpUserClipping;
...
void main()
{
    ...
    else if (bumpMorphing != 0.0)
    {
        vertex = applyMorphing(bumpMorphing, bumpTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    // note: texture coords always needed for this effect for the emboss 2d texture in the fragment shader
    // if (bumpTexturing)
    gl_TexCoord[0] = (bumpSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}