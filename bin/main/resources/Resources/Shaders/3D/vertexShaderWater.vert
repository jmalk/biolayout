...
uniform bool waterSphericalMapping;
...
void main()
{
    ...
    {
        vertex = applyMorphing(waterMorphing, waterTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    // note: texture coords always needed for this effect for the water 2d texture in the fragment shader
    // if (waterTexturing)
    gl_TexCoord[0] = (waterSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}