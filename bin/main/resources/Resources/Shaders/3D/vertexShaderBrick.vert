...
uniform bool brickSphericalMapping;
uniform float brickMorphing;
uniform bool brickUserClipping;
uniform float brickTimer;
...
void main()
{
    ...
    else if (brickMorphing != 0.0)
    {
        vertex = applyMorphing(brickMorphing, brickTimer);
    }
    else
    {
        vertex = gl_Vertex;
    ...
    VS_MC_POSITION = vec3(vertex.xyz) / animationNodeSizeRatio;

    gl_Position = gl_ModelViewProjectionMatrix * vertex;

    if (brickUserClipping)
    {
    ...
    if (brickTexturing)
        gl_TexCoord[0] = (brickSphericalMapping) ? vec4(applySphericalCoordinates(VS_POSITION, VS_NORMAL), (gl_TextureMatrix[0] * gl_MultiTexCoord0).pq)  : gl_TextureMatrix[0] * gl_MultiTexCoord0;
}